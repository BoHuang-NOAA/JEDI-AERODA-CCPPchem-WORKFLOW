...
while [ ${imem} -le ${nmem} ]; do
    ...
    while [ ${itile} -le 6 ]; do
       ...
       tilefileanlout=${workanal}/${memstr}/${nowfilestr}.getkf_aero.fv_tracer.res.tile${tilestr}.nc
...
# Create yaml file
cat << EOF > ${WorkDir}/jedigetkf_gfs_aero.yaml
geometry:
...
output:
  ...
  filename_core: getkf_aero.fv_core.res.nc
  filename_trcr: getkf_aero.fv_tracer.res.nc
  filename_cplr: getkf_aero.coupler.res
  first: PT0H
  frequency: PT1H
  date: *date
EOF
